...
- launch/: Launch file for orchestrating the simulator node (only).
...
3. Option B: Use the launch file (for simulator only):

   The launch file checks if the simulator is already running and starts it only if necessary.
   It does NOT run the teleop node, as the teleop requires direct terminal input (TTY).

   ros2 launch custom_turtlesim custom_turtlesim.launch.py

   You can still run the teleop manually in another terminal after starting the launch file.
...
- Turtle spawn position (x, y, theta) can be configured via the 'x', 'y', 'theta' parameters.
...
- The launch file is intended for the simulator only. Teleop nodes must be started manually in a TTY-enabled terminal.
